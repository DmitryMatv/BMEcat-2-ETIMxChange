...
def convert_BMEcat(XML_path):
    ...
    def get_val_attr(target_field, XML_root, val_type=str, **attributes):
        ...
        if attributes:
            ...
            # Escape single quotes in values to prevent XPath injection
            conditions = []
    ...
    xChange = {
        "CatalogueId": get_val("CATALOG_ID", HEADER)
        or get_val("CATALOG_NAME", HEADER),  # Mandatory
        "CatalogueName": [],
        ...
        # "DatapoolName": "",
        # "DatapoolGln": "",
        "CatalogueValidityStart": get_val("DATE", HEADER)
        or get_val("UDX.EDXF.VALID_FROM", CATALOG)
        or "1971-08-15",  # Mandatory
    ...
    def convert_to_language_region_code(iso_code: Optional[str]) -> Optional[str]:
        ...
        if iso_code not in iso_to_lang_region:
            # Log warning and return None for unsupported codes
            print(f"Warning: Unsupported ISO 639-2 code: {iso_code}")
            return None
    ...
    def append_multilingual_elements(
        parent_element, xpath_query, target, entry_key, default_lang=DefaultLangxChange
    ):
        for element in parent_element.iterfind(xpath_query):
            lang_attr = element.xpath("@lang")
            language = (
                convert_to_language_region_code(lang_attr[0])
                if lang_attr
                else default_lang
            )
            target.append(
    ...
    append_multilingual_elements(
        HEADER, ".//CATALOG_NAME", xChange["CatalogueName"], "CatalogueName"
    )
    ...
    for SUPPLIER_MIME in HEADER.iterfind(".//MIME"):
        SupplierAttachments = {
            "AttachmentType": map_attachment_type(get_val("MIME_DESCR", SUPPLIER_MIME))
            or "ATX099",  # Mandatory
            "AttachmentDetails": [
                {
                    "AttachmentLanguage": [
                        DefaultLangxChange
                    ],  # Mandatory if many languages !!!
        ...
        append_multilingual_elements(
            SUPPLIER_MIME,
            "MIME_DESCR",
            SupplierAttachments["AttachmentDetails"][0]["AttachmentTypeSpecification"],
            "AttachmentTypeSpecification",
        )
        append_multilingual_elements(
            SUPPLIER_MIME,
            "MIME_DESCR",
            SupplierAttachments["AttachmentDetails"][0]["AttachmentDescription"],
            "AttachmentDescription",
        )
    ...
    for BMECAT_PRODUCT in CATALOG.iterfind("./PRODUCT"):
        ...
        append_multilingual_elements(
            ...
            ProductIdentification["BrandDetails"][0]["BrandSeries"],
            "BrandSeries",
        )
        append_multilingual_elements(
            UDX,
            "./UDX.EDXF.PRODUCT_VARIATION",
            ProductIdentification["BrandDetails"][0]["BrandSeriesVariation"],
            "BrandSeriesVariation",
        )
        Product["ProductIdentification"].update(ProductIdentification)

        ProductDetails = {
            "ProductStatus": map_product_status(
                BMECAT_PRODUCT.xpath(".//PRODUCT_STATUS/@type")[0]
            )
            if BMECAT_PRODUCT.xpath(".//PRODUCT_STATUS/@type")
            else None,
            "ProductType": get_val("PRODUCT_TYPE", BMECAT_PRODUCT).upper()
            if get_val("PRODUCT_TYPE", BMECAT_PRODUCT)
            else None,
            # "CustomisableProduct": "",
            "ProductDescriptions": [
                {
                    "DescriptionLanguage": DefaultLangxChange,
                    "MinimalProductDescription": get_val_attr(
                        "UDX.EDXF.DESCRIPTION_VERY_SHORT", UDX, no_lang=True
                    )
                    or get_val_attr("DESCRIPTION_SHORT", BMECAT_PRODUCT, no_lang=True)
                    or get_val_attr(
                        "UDX.EDXF.DESCRIPTION_VERY_SHORT", UDX, lang=DefaultLangBMEcat
                    )
                    or get_val_attr(
                        "DESCRIPTION_LONG", BMECAT_PRODUCT, lang=DefaultLangBMEcat
                    ),
                    "UniqueMainProductDescription": get_val_attr(
                        "DESCRIPTION_SHORT", BMECAT_PRODUCT, no_lang=True
                    )
                    or get_val_attr(
                        "DESCRIPTION_SHORT", BMECAT_PRODUCT, lang=DefaultLangBMEcat
                    ),
                    "FullProductDescription": get_val_attr(
                        "DESCRIPTION_LONG", BMECAT_PRODUCT, no_lang=True
                    )
                    or get_val_attr(
                        "DESCRIPTION_LONG", BMECAT_PRODUCT, lang=DefaultLangBMEcat
                    ),
                    # "ProductMarketingText": "",
                    "ProductSpecificationText": get_val_attr(
                        "UDX.EDXF.TENDER_TEXT", UDX, no_lang=True
                    )
                    or get_val_attr(
                        "UDX.EDXF.TENDER_TEXT", UDX, lang=DefaultLangBMEcat
                    ),
                    "ProductApplicationInstructions": get_val_attr(
                        "REMARKS", BMECAT_PRODUCT, no_lang=True
                    )
                    or get_val_attr("REMARKS", BMECAT_PRODUCT, lang=DefaultLangBMEcat),
                    "ProductKeyword": [
                        kw.text.strip()
                        for kw in BMECAT_PRODUCT.findall(".//KEYWORD")
                        if kw.text
                        and kw.text.strip()
                        and (
                            "lang" not in kw.attrib
                            or kw.get("lang") == DefaultLangBMEcat
                        )
                    ],
                    # "ProductPageUrl": ""
                }
            ],
            "WarrantyConsumer": get_val(
                "UDX.EDXF.WARRANTY_CONSUMER", UDX, val_type=int
            ),
            "WarrantyBusiness": get_val(
                "UDX.EDXF.WARRANTY_BUSINESS", UDX, val_type=int
            ),
        }
            ...
            for lang in LANGUAGEs:
                if lang and lang != DefaultLangBMEcat:
                    ProductDetails["ProductDescriptions"].append(
                        {
                            "DescriptionLanguage": convert_to_language_region_code(
                                lang
                            ),
                            "MinimalProductDescription": get_val_attr(
                            ...
                            or get_val_attr(
                                "DESCRIPTION_SHORT", BMECAT_PRODUCT, lang=lang
                            )
                            or get_val_attr(
                                "UDX.EDXF.DESCRIPTION_VERY_SHORT", BMECAT_PRODUCT
                            )
                            ...
                            "FullProductDescription": get_val_attr(
                                "DESCRIPTION_LONG", BMECAT_PRODUCT, lang=lang
                            ),
                            # "ProductMarketingText": "",
                            "ProductSpecificationText": get_val_attr(
                                "UDX.EDXF.TENDER_TEXT", BMECAT_PRODUCT, lang=lang
                            ),
                            "ProductApplicationInstructions": get_val_attr(
                                "REMARKS", BMECAT_PRODUCT, lang=lang
                            ),
                            "ProductKeyword": [
                                kw.text.strip()
                                for kw in BMECAT_PRODUCT.findall(
                                    f".//KEYWORD[@lang='{lang}']"
                                )
        ...
        for REFERENCE in BMECAT_PRODUCT.iterfind("./PRODUCT_REFERENCE"):
            Product["ProductRelations"].append(
                {
                    "RelatedManufacturerProductNumber": get_val(
                        "PROD_ID_TO", REFERENCE
                    ),
                    # "RelatedProductGtin": [],
                    "RelationType": map_relation_type(REFERENCE.xpath("@type")[0])
                    if REFERENCE.xpath("@type")
                    else "OTHER",
                    "RelatedProductQuantity": int(REFERENCE.xpath("@quantity")[0])
                    if REFERENCE.xpath("@quantity")
                    else 1,
                }
            )
        ...
        Legislation = {
            ...
            "BatteryContained": get_val(
                "UDX.EDXF.BATTERY_CONTAINED", UDX, val_type=bool
            ),
            # "WeeeCategory": "",
            "RohsIndicator": get_val("UDX.EDXF.ROHS_INDICATOR", UDX),
            "CeMarking": get_val("UDX.EDXF.CE_MARKING", UDX, val_type=bool),
            "SdsIndicator": get_val_attr(
                "SPECIAL_TREATMENT_CLASS", BMECAT_PRODUCT, val_type=bool, type="SDS"
            ),
            ...
            "NetWeightHazardousSubstances": get_val(
                "UDX.EDXF.NET_WEIGHT_OF_HAZARDOUS_SUBSTANCE", UDX
            ),
            "VolumeHazardousSubstances": get_val(
                "UDX.EDXF.VOLUME_OF_HAZARDOUS_SUBSTANCES", UDX
            ),
            "UnShippingName": [],
            "PackingGroup": get_val("UDX.EDXF.PACKING_GROUP", UDX),
            "LimitedQuantities": get_val(
                "UDX.EDXF.LIMITED_QUANTITIES", UDX, val_type=bool
            ),
            "ExceptedQuantities": get_val(
                "UDX.EDXF.EXCEPTED_QUANTITIES", UDX, val_type=bool
            ),
            ...
            "EnvironmentalHazards": get_val(
                "UDX.EDXF.ENVIRONMENTAL_HAZARDS", UDX, val_type=bool
            ),
            ...
            "PrecautionaryStatement": [
                get_val("UDX.EDXF.PRECAUTIONARY_STATEMENT", UDX)
            ],
        ...
        append_multilingual_elements(
            UDX,
            ".//UDX.EDXF.SHIPPING_NAME",
            Legislation["UnShippingName"],
            "UnShippingName",
        )
        Product["Legislation"].update(Legislation)

        for PRODUCT_MIME in UDX.iterfind(".//UDX.EDXF.MIME"):
            mime_source_elem = PRODUCT_MIME.find("UDX.EDXF.MIME_SOURCE")
            mime_lang = (
                mime_source_elem.get("lang", None)
                if mime_source_elem is not None
                else None
            )
            ProductAttachments = {
                "AttachmentType": map_attachment_type(
                    get_val("UDX.EDXF.MIME_CODE", PRODUCT_MIME)
                )
                or "ATX099",  # Mandatory
                # "ProductImageSimilar": "",
                "AttachmentOrder": get_val(
                    "UDX.EDXF.MIME_ORDER", PRODUCT_MIME, val_type=int
                ),
                "AttachmentDetails": [
                    {
                        "AttachmentLanguage": [
                            convert_to_language_region_code(mime_lang)
                            or DefaultLangxChange
                        ],  # Mendatory if many languages !!!
                        # "AttachmentTypeSpecification": "",
                        "AttachmentFilename": get_val(
                            "UDX.EDXF.MIME_FILENAME", PRODUCT_MIME
                        ),
                        "AttachmentUri": get_val("UDX.EDXF.MIME_SOURCE", PRODUCT_MIME),
                        "AttachmentDescription": [],
                        "AttachmentIssueDate": get_val(
                            "UDX.EDXF.MIME_ISSUE_DATE", PRODUCT_MIME
                        ),
                        "AttachmentExpiryDate": get_val(
                            "UDX.EDXF.MIME_EXPIRY_DATE", PRODUCT_MIME
                        ),
                    }
                ],
            }
            append_multilingual_elements(
                ...
                ProductAttachments["AttachmentDetails"][0]["AttachmentDescription"],
                "AttachmentDescription",
            )
            Product["ProductAttachments"].append(ProductAttachments)

        if (
            BMECAT_PRODUCT.find(".//MIME") is not None
        ):  # Specially for some ABB catalogs
            for PRODUCT_MIME_2 in BMECAT_PRODUCT.iterfind(".//MIME"):
                mime_source_elem_2 = PRODUCT_MIME_2.find("MIME_SOURCE")
                ProductAttachments = {
                    "AttachmentType": map_attachment_type(
                        get_val("MIME_CODE", PRODUCT_MIME_2)
                        or get_val("MIME_DESCR", PRODUCT_MIME_2)
                    )
                    or "ATX099",  # Mandatory
                    # "ProductImageSimilar": "",
                    "AttachmentOrder": get_val(
                        "MIME_ORDER", PRODUCT_MIME_2, val_type=int
                    ),
                    "AttachmentDetails": [
                        {
                            "AttachmentLanguage": [
                                convert_to_language_region_code(
                                    mime_source_elem_2.get("lang", None)
                                    if mime_source_elem_2 is not None
                                    else None
                                )
                                or DefaultLangxChange
                            ],  # Mendatory if many languages !!!
                            # "AttachmentTypeSpecification": "",
                            "AttachmentFilename": get_val(
                                "MIME_FILENAME", PRODUCT_MIME_2
                            ),
                            "AttachmentUri": get_val("MIME_SOURCE", PRODUCT_MIME_2),
                            "AttachmentDescription": [],
                            "AttachmentIssueDate": get_val(
                                "MIME_ISSUE_DATE", PRODUCT_MIME_2
                            ),
                            "AttachmentExpiryDate": get_val(
                                "MIME_EXPIRY_DATE", PRODUCT_MIME_2
                            ),
                        }
                    ],
                }
        ...
        for PRODUCT_FEATURES in BMECAT_PRODUCT.iterfind("./PRODUCT_FEATURES"):
            EtimClassification = {
                "EtimReleaseVersion": extract_etim_version(
                    get_val("REFERENCE_FEATURE_SYSTEM_NAME", PRODUCT_FEATURES)
                ),  # Mandatory
                "EtimClassCode": extract_class_code(
                    get_val("REFERENCE_FEATURE_GROUP_ID", PRODUCT_FEATURES)
                ),  # Mandatory
                # "EtimClassVersion": "",
                "EtimDynamicReleaseDate": get_val(
                    "UDX.EDXF.PRODUCT_ETIM_RELEASE_DATE", UDX
                ),
                "EtimFeatures": [],
                "EtimModellingClassCode": get_val(
                    "UDX.EDXF.REFERENCE_FEATURE_MC_ID", UDX
                ),
                "EtimModellingClassVersion": get_val(
                    "UDX.EDXF.REFERENCE_FEATURE_MC_VERSION", UDX, val_type=int
                ),
                "EtimModellingPorts": [],
            }
            ...
            for PortCode in ModellingPorts:
                EtimModellingPort = {
                    "EtimModellingPortcode": int(PortCode)
                    if PortCode.isdigit()
                    else None,
                ...
                for FEATURE_MC in UDX.iterfind(
                    f".//UDX.EDXF.FEATURE_MC[UDX.EDXF.PORTCODE='{PortCode}']"
                ):
                    EtimFeature = {
                        "EtimFeatureCode": get_val(
                            "UDX.EDXF.FNAME", FEATURE_MC
                        ),  # Mandatory
                        "EtimValueCoordinateX": get_val(
                            "UDX.EDXF.COORDINATE_X", FEATURE_MC
                        ),
                        "EtimValueCoordinateY": get_val(
                            "UDX.EDXF.COORDINATE_Y", FEATURE_MC
                        ),
                        "EtimValueCoordinateZ": get_val(
                            "UDX.EDXF.COORDINATE_Z", FEATURE_MC
                        ),
                    }
                    ...
                    EtimFeature.update(
                        process_feature_values(
                            FEATURE_MC, feature_xpath="./UDX.EDXF.FVALUE"
                        )
                    )
        ...
        OtherClassifications = {
            "ClassificationName": "FUNIT"
            if get_val("FUNIT", BMECAT_PRODUCT)
            else "",  # WIP
            # "ClassificationVersion": "",
            "ClassificationClassCode": get_val("FUNIT", BMECAT_PRODUCT),  # WIP
            "ClassificationFeatures": [
                {
                    "ClassificationFeatureName": "FUNIT"
                    if get_val("FUNIT", BMECAT_PRODUCT)
                    else "",  # WIP
                    "ClassificationFeatureValue1": get_val(
                        "FUNIT", BMECAT_PRODUCT
                    ),  # WIP
        ...
        for PRODUCT_CHARACTERISTIC in UDX.iterfind(
            ".//UDX.EDXF.PRODUCT_CHARACTERISTIC"
        ):
            ProductCountrySpecificFields = {
                "CSProductCharacteristicCode": get_val(
                    "UDX.EDXF.PRODUCT_CHARACTERISTIC_CODE", PRODUCT_CHARACTERISTIC
                ),
                "CSProductCharacteristicName": [],
                "CSProductCharacteristicValueBoolean": get_val(
                    "UDX.EDXF.PRODUCT_CHARACTERISTIC_VALUE_BOOLEAN",
                    PRODUCT_CHARACTERISTIC,
                    val_type=bool,
                ),
                "CSProductCharacteristicValueNumeric": get_val(
                    "UDX.EDXF.PRODUCT_CHARACTERISTIC_VALUE_NUMERIC",
                    PRODUCT_CHARACTERISTIC,
                ),
                "CSProductCharacteristicValueRangeLower": get_val(
                    "UDX.EDXF.PRODUCT_CHARACTERISTIC_VALUE_RANGE_FROM",
                    PRODUCT_CHARACTERISTIC,
                ),
                "CSProductCharacteristicValueRangeUpper": get_val(
                    "UDX.EDXF.PRODUCT_CHARACTERISTIC_VALUE_RANGE_TO",
                    PRODUCT_CHARACTERISTIC,
                ),
                "CSProductCharacteristicValueString": [],
                "CSProductCharacteristicValueSet": [],
                "CSProductCharacteristicValueSelect": get_val(
                    "UDX.EDXF.PRODUCT_CHARACTERISTIC_VALUE_SELECT",
                    PRODUCT_CHARACTERISTIC,
                ),
                "CSProductCharacteristicValueUnitCode": get_val(
                    "UDX.EDXF.PRODUCT_CHARACTERISTIC_VALUE_UNIT_CODE",
                    PRODUCT_CHARACTERISTIC,
                ),
                "CSProductCharacteristicReferenceGtin": get_val(
            ...
            append_multilingual_elements(
                ...
                ProductCountrySpecificFields["CSProductCharacteristicName"],
                "CSProductCharacteristicName",
            )
            ...
            append_multilingual_elements(
                ...
                ProductCountrySpecificFields["CSProductCharacteristicValueString"],
                "CSProductCharacteristicValueString",
            )
            ...
            append_multilingual_elements(
                ...
                ProductCountrySpecificFields["CSProductCharacteristicValueSet"],
                "CSProductCharacteristicValueSet",
            )
        ...
        TradeItem = {
            "ItemIdentification": {
                ...
                "ItemGtin": [
                    get_val_attr("INTERNATIONAL_PID", BMECAT_PRODUCT, type="gtin")
                ],
                "BuyerItemNumber": get_val_attr(
                    "BUYER_PID", BMECAT_PRODUCT, type="buyer_specific"
                ),
            ...
            "ItemDetails": {
                # "ItemStatus": "",
                "ItemCondition": (
                    BMECAT_PRODUCT.xpath(".//PRODUCT_STATUS/@type")[0].upper()
                    if BMECAT_PRODUCT.xpath(".//PRODUCT_STATUS/@type")
                    and BMECAT_PRODUCT.xpath(".//PRODUCT_STATUS/@type")[0].lower()
                    in ["new", "refurbished", "used"]
                    else None
                ),
                "StockItem": get_val("UDX.EDXF.PRODUCT_TO_STOCK", UDX, val_type=bool),
                "ShelfLifePeriod": get_val(
                    "UDX.EDXF.SHELF_LIFE_PERIOD", UDX, val_type=int
                ),
                "ItemDescriptions": [
                    {
                        "MinimalItemDescription": get_val_attr(
                            "UDX.EDXF.DESCRIPTION_VERY_SHORT", UDX
                        )
            ...
            "Ordering": {
                ...
                "StandardOrderLeadTime": get_val(
                    "DELIVERY_TIME", BMECAT_PRODUCT, val_type=int
                ),
            ...
            TradeItem["ItemRelations"].append(
                {
                    ...
                    # "RelatedItemGtin": [],
                    "RelationType": map_relation_type(ITEM_REFERENCE.xpath("@type")[0])
                    if ITEM_REFERENCE.xpath("@type")
                    else "OTHER",
                    "RelatedItemQuantity": int(ITEM_REFERENCE.xpath("@quantity")[0])
                    if ITEM_REFERENCE.xpath("@quantity")
                    else 1,
                }
            )
        ...
        Pricing = {
            "PriceUnit": get_val("PRICE_UNIT", BMECAT_PRODUCT)
            or get_val("ORDER_UNIT", BMECAT_PRODUCT),
            ...
            "GrossListPrice": next(
                iter(
                    BMECAT_PRODUCT.xpath(
                        './/PRODUCT_PRICE[@price_type="net_list"]/PRICE_AMOUNT/text()'
                    )
                ),
                None,
            ),
            "NetPrice": next(
                iter(
                    BMECAT_PRODUCT.xpath(
                        './/PRODUCT_PRICE[@price_type="net_customer"]/PRICE_AMOUNT/text()'
                    )
                ),
                None,
            ),
            "RecommendedRetailPrice": next(
                iter(
                    BMECAT_PRODUCT.xpath(
                        './/PRODUCT_PRICE[@price_type="nrp"]/PRICE_AMOUNT/text()'
                    )
                ),
                None,
            ),
            "Vat": get_val("TAX", BMECAT_PRODUCT),
            "PriceValidityDate": next(
                iter(
                    BMECAT_PRODUCT.xpath(
                        './/DATETIME[@type="valid_start_date"]/DATE/text()'
                    )
                ),
                None,
            ),
            "PriceExpiryDate": next(
                iter(
                    BMECAT_PRODUCT.xpath(
                        './/DATETIME[@type="valid_end_date"]/DATE/text()'
                    )
                ),
                None,
            ),
        ...
        for ITEM_MIME in UDX.iterfind(".//UDX.EDXF.MIME"):
            ItemAttachments = {
                "AttachmentType": map_attachment_type(
                    get_val("UDX.EDXF.MIME_CODE", ITEM_MIME)
                )
                or "ATX099",  # Mandatory
                "AttachmentDetails": [
                    {
                        ...
                        # "AttachmentTypeSpecification": "",
                        "AttachmentFilename": get_val(
                            "UDX.EDXF.MIME_FILENAME", ITEM_MIME
                        ),
                        "AttachmentUri": get_val("UDX.EDXF.MIME_SOURCE", ITEM_MIME),
                        "AttachmentDescription": [],
                        "AttachmentIssueDate": get_val(
                            "UDX.EDXF.MIME_ISSUE_DATE", ITEM_MIME
                        ),
                        "AttachmentExpiryDate": get_val(
                            "UDX.EDXF.MIME_EXPIRY_DATE", ITEM_MIME
                        ),
                    }
                ],
            }
            append_multilingual_elements(
                ITEM_MIME,
                "UDX.EDXF.MIME_DESIGNATION",
                ItemAttachments["AttachmentDetails"][0]["AttachmentDescription"],
                "AttachmentDescription",
            )
        ...
        for ITEM_CHARACTERISTIC in UDX.iterfind(".//UDX.EDXF.PRODUCT_CHARACTERISTIC"):
            ProductCountrySpecificFields = {
                "CSProductCharacteristicCode": get_val(
                    "UDX.EDXF.PRODUCT_CHARACTERISTIC_CODE", ITEM_CHARACTERISTIC
                ),
                "CSProductCharacteristicName": [],
                "CSProductCharacteristicValueBoolean": get_val(
                    "UDX.EDXF.PRODUCT_CHARACTERISTIC_VALUE_BOOLEAN",
                ...
                "CSProductCharacteristicValueNumeric": get_val(
                    "UDX.EDXF.PRODUCT_CHARACTERISTIC_VALUE_NUMERIC", ITEM_CHARACTERISTIC
                ),
                "CSProductCharacteristicValueRangeLower": get_val(
                    "UDX.EDXF.PRODUCT_CHARACTERISTIC_VALUE_RANGE_FROM",
                    ITEM_CHARACTERISTIC,
                ...
                "CSProductCharacteristicValueSet": [],
                "CSProductCharacteristicValueSelect": get_val(
                    "UDX.EDXF.PRODUCT_CHARACTERISTIC_VALUE_SELECT", ITEM_CHARACTERISTIC
                ),
                "CSProductCharacteristicValueUnitCode": get_val(
                    "UDX.EDXF.PRODUCT_CHARACTERISTIC_VALUE_UNIT_CODE",
                    ITEM_CHARACTERISTIC,
            ...
            TradeItem["PackagingUnit"].append(
                {
                    "PackagingIdentification": {
                        ...
                        "PackagingTypeCode": get_val(
                            "UDX.EDXF.PACKING_UNIT_CODE", PACKING_UNIT
                        ),
                        "PackagingQuantity": get_val(
                            "UDX.EDXF.QUANTITY_MAX", PACKING_UNIT
                        ),
                        # TradeItemPrimaryPackaging
                        "PackagingGs1Code128": get_val(
                            "UDX.EDXF.GS1_128", PACKING_UNIT
                        ),
                        # PackagingRecyclable
                        # PackagingReusable
                        "PackagingBreak": get_val(
                            "UDX.EDXF.PACKAGE_BREAK", PACKING_UNIT, val_type=bool
                        ),
                        "NumberOfPackagingParts": get_val(
                            "UDX.EDXF.PACKING_PARTS", PACKING_UNIT, val_type=int
                        ),
                    },
                    "PackagingLogisticDetails": [
                        {
                            ...
                            # PackagingPartGtin
                            "PackagingTypeLength": get_val(
                                "UDX.EDXF.LENGTH", PACKING_UNIT
                            ),
                            "PackagingTypeWidth": get_val(
                                "UDX.EDXF.WIDTH", PACKING_UNIT
                            ),
                            "PackagingTypeHeight": get_val(
                                "UDX.EDXF.DEPTH", PACKING_UNIT
                            ),
                            "PackagingTypeDiameter": get_val(
                                "UDX.EDXF.DIAMETER", PACKING_UNIT
                            ),
                            # PackagingTypeDimensionUnit
                            "PackagingTypeWeight": get_val(
                                "UDX.EDXF.WEIGHT", PACKING_UNIT
                            ),
...
def convert_file(input_path: str, output_path: str) -> None:
    ...
    with open(output_path, "wb") as file:
        file.write(orjson.dumps(xChange_cleaned, option=orjson.OPT_INDENT_2))
    del xChange_cleaned
